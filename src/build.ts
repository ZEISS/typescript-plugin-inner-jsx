...
import createInnerJsxTransformer from './createTransformer';
...
class Compiler {
  ...
  constructor(args: { tsconfigPath?: string; commandLine: Array<string> }) {
  ...
  private createConfigFileHost() {
    ...
    const configFileHost: ParseConfigFileHost = {
      onUnRecoverableConfigFileDiagnostic(diagnostic: Diagnostic) {
        console.error(formatDiagnostic(diagnostic, diagnosticsHost));
      },
  ...
  private createConfig(tsconfigPath: string = path.join(__dirname, 'tsconfig.json'), commandLine: Array<string>) {
